 ...
 236                     ; 122         PWR->CSR1 |= PWR_CSR1_PVDE;
 238  001f 721050b2      	bset	20658,#0
 240  0023 2004          	jra	L311
 241  0025               L111:
 242                     ; 125         PWR->CSR1 &= (uint8_t)(~PWR_CSR1_PVDE);
 244  0025 721150b2      	bres	20658,#0
 245  0029               L311:
 246                     ; 127 }
 249  0029 81            	ret
 284                     ; 162 void PWR_FastWakeUpCmd(FunctionalState NewState)
 284                     ; 163 {
 285                     	switch	.text
 286  002a               _PWR_FastWakeUpCmd:
 290                     ; 165     assert_param(IS_FUNCTIONAL_STATE(NewState));
 292                     ; 167     if (NewState != DISABLE) {
 294  002a 4d            	tnz	a
 295  002b 2706          	jreq	L331
 296                     ; 169         PWR->CSR2 |= PWR_CSR2_FWU;
 298  002d 721450b3      	bset	20659,#2
 300  0031 2004          	jra	L531
 301  0033               L331:
 302                     ; 172         PWR->CSR2 &= (uint8_t)(~PWR_CSR2_FWU);
 304  0033 721550b3      	bres	20659,#2
 305  0037               L531:
 306                     ; 174 }
 309  0037 81            	ret
 345                     ; 182 void PWR_UltraLowPowerCmd(FunctionalState NewState)
 345                     ; 183 {
 346                     	switch	.text
 347  0038               _PWR_UltraLowPowerCmd:
 351                     ; 185     assert_param(IS_FUNCTIONAL_STATE(NewState));
 353                     ; 187     if (NewState != DISABLE) {
 355  0038 4d            	tnz	a
 356  0039 2706          	jreq	L551
 357                     ; 189         PWR->CSR2 |= PWR_CSR2_ULP;
 359  003b 721250b3      	bset	20659,#1
 361  003f 2004          	jra	L751
 362  0041               L551:
 363                     ; 192         PWR->CSR2 &= (uint8_t)(~PWR_CSR2_ULP);
 365  0041 721350b3      	bres	20659,#1
 366  0045               L751:
 367                     ; 194 }
 370  0045 81            	ret
 405                     ; 217 void PWR_PVDITConfig(FunctionalState NewState)
 405                     ; 218 {
 406                     	switch	.text
 407  0046               _PWR_PVDITConfig:
 411                     ; 220     assert_param(IS_FUNCTIONAL_STATE(NewState));
 413                     ; 222     if (NewState != DISABLE) {
 415  0046 4d            	tnz	a
 416  0047 2706          	jreq	L771
 417                     ; 224         PWR->CSR1 |= PWR_CSR1_PVDIEN;
 419  0049 721850b2      	bset	20658,#4
 421  004d 2004          	jra	L102
 422  004f               L771:
 423                     ; 227         PWR->CSR1 &= (uint8_t)(~PWR_CSR1_PVDIEN);
 425  004f 721950b2      	bres	20658,#4
 426  0053               L102:
 427                     ; 229 }
 430  0053 81            	ret
 523                     ; 243 FlagStatus PWR_GetFlagStatus(PWR_FLAG_TypeDef PWR_FLAG)
 523                     ; 244 {
 524                     	switch	.text
