 ...
 233                     ; 144 uint8_t atomMutexCreate (ATOM_MUTEX *mutex)
 233                     ; 145 {
 235                     	switch	.text
 ...
 395  003d f7            ld (X),A
 396  003e 9b            sim
 398                     ; 218             tcb_ptr = tcbDequeueHead (&mutex->suspQ);
 ...
 408                     ; 224                 tcb_ptr->suspend_wake_status = ATOM_ERR_DELETED;
 410  004a 1e04          	ldw	x,(OFST-1,sp)
 411  004c a6ca          	ld	a,#202
 412  004e e70e          	ld	(14,x),a
 413                     ; 227                 if (tcbEnqueuePriority (&tcbReadyQ, tcb_ptr) != ATOM_OK)
 ...
 734  0104               L41:
 735                     ; 380             if (timeout >= 0)
 737  0104 9c            	rvf
 738  0105 0d17          	tnz	(OFST+5,sp)
 739  0107 2e03          	jrsge	L61
 740  0109 cc01a4        	jp	L353
 741  010c               L61:
 742                     ; 383                 if (tcbEnqueuePriority (&mutex->suspQ, curr_tcb_ptr) != ATOM_OK)
 ...
 937  01d9 86            pop CC
 939  01da               L343:
 940                     ; 516     return (status);
 942  01da 7b10          	ld	a,(OFST-2,sp)
 945  01dc 5b14          	addw	sp,#20
 946  01de 81            	ret
1032                     ; 546 uint8_t atomMutexPut (ATOM_MUTEX * mutex)
1032                     ; 547 {
1033                     	switch	.text
...
1064  01fa f7            ld (X),A
1065  01fb 9b            sim
1067                     ; 567         if (mutex->owner != curr_tcb_ptr)
...
1315  02af f7            ld (X),A
1316  02b0 9b            sim
1318                     ; 699         timer_data_ptr->tcb_ptr->suspend_wake_status = ATOM_TIMEOUT;
1320  02b1 1e02          	ldw	x,(OFST-1,sp)
1321  02b3 fe            	ldw	x,(x)
1322  02b4 a602          	ld	a,#2
1323  02b6 e70e          	ld	(14,x),a
1324                     ; 702         timer_data_ptr->tcb_ptr->suspend_timo_cb = NULL;
1326  02b8 1e02          	ldw	x,(OFST-1,sp)
1327  02ba fe            	ldw	x,(x)
1328  02bb 905f          	clrw	y
1329  02bd ef0f          	ldw	(15,x),y
1330                     ; 705         (void)tcbDequeueEntry (&timer_data_ptr->mutex_ptr->suspQ, timer_data_ptr->tcb_ptr);
...
1339  02ca 85            	popw	x
1340                     ; 708         (void)tcbEnqueuePriority (&tcbReadyQ, timer_data_ptr->tcb_ptr);
