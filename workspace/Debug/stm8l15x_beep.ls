 ...
 129                     ; 107         BEEP->CSR2 &= (uint8_t)(~BEEP_CSR2_BEEPDIV); /* Clear bits */
 131  0013 c650f3        	ld	a,20723
 132  0016 a4e0          	and	a,#224
 133  0018 c750f3        	ld	20723,a
 134                     ; 108         BEEP->CSR2 |= BEEP_CALIBRATION_DEFAULT;
 136  001b 721050f3      	bset	20723,#0
 137  001f               L15:
 138                     ; 112     BEEP->CSR2 &= (uint8_t)(~BEEP_CSR2_BEEPSEL);
 140  001f c650f3        	ld	a,20723
 141  0022 a43f          	and	a,#63
 142  0024 c750f3        	ld	20723,a
 143                     ; 113     BEEP->CSR2 |= (uint8_t)(BEEP_Frequency);
 ...
 151  002f 84            	pop	a
 152  0030 81            	ret
 207                     ; 123 void BEEP_Cmd(FunctionalState NewState)
 207                     ; 124 {
 208                     	switch	.text
 209  0031               _BEEP_Cmd:
 213                     ; 126     assert_param(IS_FUNCTIONAL_STATE(NewState));
 215                     ; 128     if (NewState != DISABLE) {
 217  0031 4d            	tnz	a
 218  0032 2706          	jreq	L101
 219                     ; 130         BEEP->CSR2 |= BEEP_CSR2_BEEPEN;
 221  0034 721a50f3      	bset	20723,#5
 223  0038 2004          	jra	L301
 224  003a               L101:
 225                     ; 133         BEEP->CSR2 &= (uint8_t)(~BEEP_CSR2_BEEPEN);
 227  003a 721b50f3      	bres	20723,#5
 228  003e               L301:
 229                     ; 135 }
 232  003e 81            	ret
 268                     ; 168 void BEEP_LSClockToTIMConnectCmd(FunctionalState NewState)
 268                     ; 169 {
 269                     	switch	.text
 270  003f               _BEEP_LSClockToTIMConnectCmd:
 274                     ; 171     assert_param(IS_FUNCTIONAL_STATE(NewState));
 276                     ; 173     if (NewState != DISABLE) {
 278  003f 4d            	tnz	a
 279  0040 2706          	jreq	L321
 280                     ; 175         BEEP->CSR1 |= BEEP_CSR1_MSR;
 282  0042 721050f0      	bset	20720,#0
 284  0046 2004          	jra	L521
 285  0048               L321:
 286                     ; 178         BEEP->CSR1 &= (uint8_t)(~BEEP_CSR1_MSR);
 ...
 349                     ; 194 void BEEP_LSICalibrationConfig(uint32_t LSIFreqHz)
 349                     ; 195 {
 350                     	switch	.text
 351  004d               _BEEP_LSICalibrationConfig:
 353  004d 5206          	subw	sp,#6
 354       00000006      OFST:	set	6
 357                     ; 200     assert_param(IS_LSI_FREQUENCY(LSIFreqHz));
 359                     ; 202     lsifreqkhz = (uint16_t)(LSIFreqHz / 1000); /* Converts value in kHz */
 ...
 370                     ; 206     BEEP->CSR2 &= (uint8_t)(~BEEP_CSR2_BEEPDIV); /* Clear bits */
 372  0060 c650f3        	ld	a,20723
 373  0063 a4e0          	and	a,#224
 374  0065 c750f3        	ld	20723,a
 375                     ; 208     A = (uint16_t)(lsifreqkhz >> 3U); /* Division by 8, keep integer part only */
 377  0068 1e03          	ldw	x,(OFST-3,sp)
 378  006a 54            	srlw	x
 379  006b 54            	srlw	x
 380  006c 54            	srlw	x
 381  006d 1f05          	ldw	(OFST-1,sp),x
 382                     ; 210     if ((8U * A) >= ((lsifreqkhz - (8U * A)) * (1U + (2U * A)))) {
 ...
 403                     ; 211         BEEP->CSR2 |= (uint8_t)(A - 2U);
 ...
 411  009f               L551:
 412                     ; 213         BEEP->CSR2 |= (uint8_t)(A - 1U);
