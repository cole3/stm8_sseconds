 ...
 416  002c               L12:
 417                     ; 147         case 1:
 417                     ; 148             WFE->CR1 |= (uint8_t)WFE_Source;
 ...
 425  0036               L32:
 426                     ; 151         case 2:
 426                     ; 152             WFE->CR2 |= (uint8_t)WFE_Source;
 ...
 434  0040               L52:
 435                     ; 155         case 3:
 435                     ; 156             WFE->CR3 |= (uint8_t)WFE_Source;
 ...
 443  004a               L72:
 444                     ; 159         case 4:
 444                     ; 160             WFE->CR4 |= (uint8_t)WFE_Source;
 ...
 474  0068               L33:
 475                     ; 168         case 1:
 475                     ; 169             WFE->CR1 &= (uint8_t)(~(uint8_t)WFE_Source);
 ...
 488  0074               L53:
 489                     ; 172         case 2:
 489                     ; 173             WFE->CR2 &= (uint8_t)(~ (uint8_t)WFE_Source);
 ...
 498  007f               L73:
 499                     ; 176         case 3:
 499                     ; 177             WFE->CR3 &= (uint8_t)(~(uint8_t)WFE_Source);
 ...
 508  008a               L14:
 509                     ; 180         case 4:
 509                     ; 181             WFE->CR4 &= (uint8_t)(~(uint8_t)WFE_Source);
 ...
 570                     ; 226 FunctionalState WFE_GetWakeUpSourceEventStatus(WFE_Source_TypeDef WFE_Source)
 570                     ; 227 {
 571                     	switch	.text
 ...
 579                     ; 228     FunctionalState status = DISABLE;
 581  009b 0f01          	clr	(OFST+0,sp)
 582                     ; 230     assert_param(IS_WFE_SOURCE(WFE_Source));
 584                     ; 232     switch (WFE_Source) {
 587                     ; 302     default:
 587                     ; 303         break;
 ...
 661  0140               L122:
 662                     ; 233     case WFE_Source_TIM2_EV0:
 662                     ; 234     case WFE_Source_TIM2_EV1:
 662                     ; 235     case WFE_Source_TIM1_EV0:
 662                     ; 236     case WFE_Source_TIM1_EV1:
 662                     ; 237     case WFE_Source_EXTI_EV0:
 662                     ; 238     case WFE_Source_EXTI_EV1:
 662                     ; 239     case WFE_Source_EXTI_EV2:
 662                     ; 240     case WFE_Source_EXTI_EV3:
 662                     ; 241 
 662                     ; 242         if ((WFE->CR1 & (uint8_t)WFE_Source) != (uint8_t)0x00) {
 664  0140 c650a6        	ld	a,20646
 665  0143 1503          	bcp	a,(OFST+2,sp)
 666  0145 2706          	jreq	L162
 667                     ; 244             status = ENABLE;
 669  0147 a601          	ld	a,#1
 670  0149 6b01          	ld	(OFST+0,sp),a
 672  014b 2035          	jra	L752
 673  014d               L162:
 674                     ; 247             status = DISABLE;
 676  014d 0f01          	clr	(OFST+0,sp)
 677  014f 2031          	jra	L752
 678  0151               L322:
 679                     ; 251     case WFE_Source_EXTI_EV4:
 679                     ; 252     case WFE_Source_EXTI_EV5:
 679                     ; 253     case WFE_Source_EXTI_EV6:
 679                     ; 254     case WFE_Source_EXTI_EV7:
 679                     ; 255     case WFE_Source_EXTI_EVB_G:
 679                     ; 256     case WFE_Source_EXTI_EVD_H:
 679                     ; 257     case WFE_Source_EXTI_EVE_F:
 679                     ; 258     case WFE_Source_ADC1_COMP_EV:
 679                     ; 259 
 679                     ; 260         if ((WFE->CR2 & (uint8_t)WFE_Source) != (uint8_t)0x00) {
 681  0151 c650a7        	ld	a,20647
 682  0154 1503          	bcp	a,(OFST+2,sp)
 683  0156 2706          	jreq	L562
 684                     ; 262             status = ENABLE;
 686  0158 a601          	ld	a,#1
 687  015a 6b01          	ld	(OFST+0,sp),a
 689  015c 2024          	jra	L752
 690  015e               L562:
 691                     ; 265             status = DISABLE;
 693  015e 0f01          	clr	(OFST+0,sp)
 694  0160 2020          	jra	L752
 695  0162               L522:
 696                     ; 268     case WFE_Source_TIM3_EV0:
 696                     ; 269     case WFE_Source_TIM3_EV1:
 696                     ; 270     case WFE_Source_TIM4_EV:
 696                     ; 271     case WFE_Source_SPI1_EV:
 696                     ; 272     case WFE_Source_I2C1_EV:
 696                     ; 273     case WFE_Source_USART1_EV:
 696                     ; 274     case WFE_Source_DMA1CH01_EV:
 696                     ; 275     case WFE_Source_DMA1CH23_EV:
 696                     ; 276 
 696                     ; 277         if ((WFE->CR3 & (uint8_t)WFE_Source) != (uint8_t)0x00) {
 698  0162 c650a8        	ld	a,20648
 699  0165 1503          	bcp	a,(OFST+2,sp)
 700  0167 2706          	jreq	L172
 701                     ; 279             status = ENABLE;
 703  0169 a601          	ld	a,#1
 704  016b 6b01          	ld	(OFST+0,sp),a
 706  016d 2013          	jra	L752
 707  016f               L172:
 708                     ; 282             status = DISABLE;
 710  016f 0f01          	clr	(OFST+0,sp)
 711  0171 200f          	jra	L752
 712  0173               L722:
 713                     ; 286     case WFE_Source_TIM5_EV0:
 713                     ; 287     case WFE_Source_TIM5_EV1:
 713                     ; 288     case WFE_Source_AES_EV:
 713                     ; 289     case WFE_Source_SPI2_EV:
 713                     ; 290     case WFE_Source_USART2_EV:
 713                     ; 291     case WFE_Source_USART3_EV:
 713                     ; 292     case WFE_Source_RTC_CSS_EV:
 713                     ; 293 
 713                     ; 294         if ((WFE->CR4 & (uint8_t)WFE_Source) != (uint8_t)0x00) {
 715  0173 c650a9        	ld	a,20649
 716  0176 1503          	bcp	a,(OFST+2,sp)
 717  0178 2706          	jreq	L572
 718                     ; 296             status = ENABLE;
 720  017a a601          	ld	a,#1
 721  017c 6b01          	ld	(OFST+0,sp),a
 723  017e 2002          	jra	L752
 724  0180               L572:
 725                     ; 299             status = DISABLE;
 727  0180 0f01          	clr	(OFST+0,sp)
 728  0182               L132:
 729                     ; 302     default:
 729                     ; 303         break;
 731  0182               L752:
 732                     ; 305     return status;
