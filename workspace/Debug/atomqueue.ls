 ...
 299                     ; 145 uint8_t atomQueueCreate (ATOM_QUEUE *qptr, uint8_t *buff_ptr, uint32_t unit_size, uint32_t max_num_msgs)
 299                     ; 146 {
 301                     	switch	.text
 ...
 368                     ; 160         qptr->max_num_msgs = max_num_msgs;
 ...
 411                     ; 169         qptr->num_msgs_stored = 0;
 ...
 422                     ; 172         status = ATOM_OK;
 424  0097 0f01          	clr	(OFST+0,sp)
 425  0099 cc000f        	jra	L312
 508                     ; 199 uint8_t atomQueueDelete (ATOM_QUEUE *qptr)
 508                     ; 200 {
 509                     	switch	.text
 ...
 542  00b6 f7            ld (X),A
 543  00b7 9b            sim
 545                     ; 220             if (((tcb_ptr = tcbDequeueHead (&qptr->getSuspQ)) != NULL)
 545                     ; 221                 || ((tcb_ptr = tcbDequeueHead (&qptr->putSuspQ)) != NULL)) {
 ...
 562  00d0               L772:
 563                     ; 225                 tcb_ptr->suspend_wake_status = ATOM_ERR_DELETED;
 565  00d0 1e04          	ldw	x,(OFST-1,sp)
 566  00d2 a6ca          	ld	a,#202
 567  00d4 e70e          	ld	(14,x),a
 568                     ; 228                 if (tcbEnqueuePriority (&tcbReadyQ, tcb_ptr) != ATOM_OK) {
 ...
 676  013f               L762:
 677                     ; 280     return (status);
 679  013f 7b02          	ld	a,(OFST-3,sp)
 682  0141 5b07          	addw	sp,#7
 683  0143 81            	ret
 842                     ; 321 uint8_t atomQueueGet (ATOM_QUEUE *qptr, int32_t timeout, uint8_t *msgptr)
 842                     ; 322 {
 843                     	switch	.text
 ...
 880  015f f7            ld (X),A
 881  0160 9b            sim
 883                     ; 338         if (qptr->num_msgs_stored == 0) {
...
1051  021e f7            ld (X),A
1052  021f 9b            sim
1054                     ; 428                                 status = queue_remove (qptr, msgptr);
...
1303  02a3 f7            ld (X),A
1304  02a4 9b            sim
1306                     ; 524         if (qptr->num_msgs_stored == qptr->max_num_msgs) {
...
1472  0364 f7            ld (X),A
1473  0365 9b            sim
1475                     ; 616                                 status = queue_insert (qptr, msgptr);
...
1677  03f7 85            	popw	x
1678                     ; 696         (void)tcbEnqueuePriority (&tcbReadyQ, timer_data_ptr->tcb_ptr);
...
1701  040c 81            	ret
1774                     ; 731 static uint8_t queue_remove (ATOM_QUEUE *qptr, uint8_t* msgptr)
1774                     ; 732 {
1775                     	switch	.text
...
1821                     ; 743         qptr->remove_index += qptr->unit_size;
...
1877  04a1               L107:
1878                     ; 755         tcb_ptr = tcbDequeueHead (&qptr->putSuspQ);
...
1888                     ; 758             if (tcbEnqueuePriority (&tcbReadyQ, tcb_ptr) == ATOM_OK) {
...
1898                     ; 760                 tcb_ptr->suspend_wake_status = ATOM_OK;
1900  04b9 1e06          	ldw	x,(OFST-1,sp)
1901  04bb 6f0e          	clr	(14,x)
1902                     ; 763                 if ((tcb_ptr->suspend_timo_cb != NULL)
1902                     ; 764                     && (atomTimerCancel (tcb_ptr->suspend_timo_cb) != ATOM_OK)) {
...
2066  052e               L65:
2067                     ; 825         qptr->insert_index += qptr->unit_size;
...
2123  0590               L367:
2124                     ; 837         tcb_ptr = tcbDequeueHead (&qptr->getSuspQ);
...
2136                     ; 840             if (tcbEnqueuePriority (&tcbReadyQ, tcb_ptr) == ATOM_OK) {
...
2146                     ; 842                 tcb_ptr->suspend_wake_status = ATOM_OK;
2148  05aa 1e06          	ldw	x,(OFST-1,sp)
2149  05ac 6f0e          	clr	(14,x)
2150                     ; 845                 if ((tcb_ptr->suspend_timo_cb != NULL)
2150                     ; 846                     && (atomTimerCancel (tcb_ptr->suspend_timo_cb) != ATOM_OK)) {
