 ...
 230                     ; 128 uint8_t atomSemCreate (ATOM_SEM *sem, uint8_t initial_count)
 230                     ; 129 {
 232                     	switch	.text
 ...
 388  0039 f7            ld (X),A
 389  003a 9b            sim
 391                     ; 192             tcb_ptr = tcbDequeueHead (&sem->suspQ);
 ...
 401                     ; 197                 tcb_ptr->suspend_wake_status = ATOM_ERR_DELETED;
 403  0046 1e04          	ldw	x,(OFST-1,sp)
 404  0048 a6ca          	ld	a,#202
 405  004a e70e          	ld	(14,x),a
 406                     ; 200                 if (tcbEnqueuePriority (&tcbReadyQ, tcb_ptr) != ATOM_OK) {
...
1021                     ; 489                 CRITICAL_END ();
1023  01da 96            	ldw	x,sp
1024  01db 1c0001        	addw	x,#OFST-3
1026  01de f6            ld A,(X)
1027  01df 88            push A
1028  01e0 86            pop CC
1030                     ; 492                 status = ATOM_ERR_QUEUE;
1032  01e1 a6cc          	ld	a,#204
1033  01e3 6b04          	ld	(OFST+0,sp),a
1035  01e5 2054          	jra	L334
1036  01e7               L734:
1037                     ; 495                 tcb_ptr->suspend_wake_status = ATOM_OK;
1039  01e7 1e02          	ldw	x,(OFST-2,sp)
1040  01e9 6f0e          	clr	(14,x)
1041                     ; 498                 if ((tcb_ptr->suspend_timo_cb != NULL)
1041                     ; 499                     && (atomTimerCancel (tcb_ptr->suspend_timo_cb) != ATOM_OK)) {
...
1109                     ; 532                 status = ATOM_OK;
1111  0232 0f04          	clr	(OFST+0,sp)
1112  0234               L554:
1113                     ; 536             CRITICAL_END ();
...
1120  023a 86            pop CC
1122  023b               L334:
1123                     ; 540     return (status);
1125  023b 7b04          	ld	a,(OFST+0,sp)
1128  023d 5b06          	addw	sp,#6
1129  023f 81            	ret
1184                     ; 561 uint8_t atomSemResetCount (ATOM_SEM *sem, uint8_t count)
1184                     ; 562 {
1185                     	switch	.text
...
1301  0268 f7            ld (X),A
1302  0269 9b            sim
1304                     ; 608         timer_data_ptr->tcb_ptr->suspend_wake_status = ATOM_TIMEOUT;
1306  026a 1e02          	ldw	x,(OFST-1,sp)
1307  026c fe            	ldw	x,(x)
1308  026d a602          	ld	a,#2
1309  026f e70e          	ld	(14,x),a
1310                     ; 611         timer_data_ptr->tcb_ptr->suspend_timo_cb = NULL;
1312  0271 1e02          	ldw	x,(OFST-1,sp)
1313  0273 fe            	ldw	x,(x)
1314  0274 905f          	clrw	y
1315  0276 ef0f          	ldw	(15,x),y
1316                     ; 614         (void)tcbDequeueEntry (&timer_data_ptr->sem_ptr->suspQ, timer_data_ptr->tcb_ptr);
...
1325  0283 85            	popw	x
1326                     ; 617         (void)tcbEnqueuePriority (&tcbReadyQ, timer_data_ptr->tcb_ptr);
